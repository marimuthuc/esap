...
import static de.ovgu.featureide.fm.core.localization.StringTable.ENERGYEFFALT;
import static de.ovgu.featureide.fm.core.localization.StringTable.ENERGYFRIENDLY;
...
import static de.ovgu.featureide.fm.core.localization.StringTable.IS_DISALLOWED_EHUNGRY;
...
public class Feature extends AFeature {
	...
	private static String CONTEXT = " Context value ";
	...
	private static String DISALLOWED_EHUNGRY = IS_DISALLOWED_EHUNGRY;
	...
	public String createTooltip(Object... objects) {
		...
		if (!getStructure().isComposerContext()) {
			toolTip.append(getStructure().isConcrete() ? CONCRETE : ABSTRACT);
		}
		if (getStructure().isEnergyHungry()) {
			toolTip.append(ENERGYHUNGRY);
		}
		if (getStructure().isPrimary()) {
			toolTip.append(PRIMARY);
		}
		if (getStructure().isSecondary()) {
			toolTip.append(SECONDARY);
		}
		if (getStructure().isUser()) {
			toolTip.append(USER);
		}
		if (getStructure().isEnergyEffAlt()) {
			toolTip.append(ENERGYEFFALT);
		}

		if (getStructure().isEnergyFriendly()) {
			toolTip.append(ENERGYFRIENDLY);
		}
		...
		if (!getStructure().isComposerContext()) {
			...
			if (getStructure().isRoot()) {
				toolTip.append(ROOT);
			} else if (getStructure().isPrimary() || getStructure().isSecondary() || getStructure().isUser()) {
		...
		case INDETERMINATE_HIDDEN:
			...
			break;
		case DISALLOWED_EHUNGRY:
			toolTip.append(DISALLOWED_EHUNGRY);
			break;
		default:
			break;
		}
