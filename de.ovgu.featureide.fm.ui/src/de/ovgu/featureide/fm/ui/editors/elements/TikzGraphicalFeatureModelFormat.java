...
public class TikzGraphicalFeatureModelFormat extends APersistentFormat<IGraphicalFeatureModel> {
	...
	private final boolean[] legend = new boolean[11];
	...
	private void insertNodeHead(String node, IGraphicalFeatureModel object, StringBuilder str) {
		...
		if (object.getGraphicalFeature(object.getFeatureModel().getFeature(node)).getObject().getStructure().isEnergyHungry() == true) {
			str.append(",energyhungry");
			legend[7] = true;
		}

		if (object.getGraphicalFeature(object.getFeatureModel().getFeature(node)).getObject().getStructure().isEnergyFriendly() == true) {
			str.append(",energyfriendly");
			legend[8] = true;
		}
		if (object.getGraphicalFeature(object.getFeatureModel().getFeature(node)).getObject().getStructure().isEnergyEffAlt() == true) {
			str.append(",energyeffalt");
			legend[9] = true;
		}
		if (object.getGraphicalFeature(object.getFeatureModel().getFeature(node)).getObject().getStructure().isContextModeling() == true) {
			str.append(",contextmodeling");
			legend[10] = true;
		}
	...
	private static void printHead(StringBuilder str) {
		str.append("%---required packages & variable definitions------------------------------------" + lnSep + "\\usepackage{forest}" + lnSep
			...
			+ "		fill = blue!85!cyan!15," + lnSep + "		draw = drawColor" + lnSep + "	}," + lnSep + "	/tikz/energyhungry/.style={" + lnSep
			+ "		fill = blue!85!cyan!25," + lnSep + "		draw = drawColor" + lnSep + "	}," + lnSep + "	/tikz/energyfriendly/.style={" + lnSep
			+ "		fill = blue!85!cyan!10," + lnSep + "		draw = drawColor" + lnSep + "	}," + lnSep + "	/tikz/energyeffalt/.style={" + lnSep
			+ "		fill = blue!85!cyan!30," + lnSep + "		draw = drawColor" + lnSep + "	}," + lnSep + "	/tikz/contextmodeling/.style={" + lnSep
			+ "		fill = blue!20!cyan!20," + lnSep + "		draw = drawColor" + lnSep + "	}," + lnSep + "	mandatory/.style={" + lnSep
	...
	private void printLegend(StringBuilder str) {
		...
		if (legend[7]) {
			check = true;
			myString.append("		\\node [energyhungry,label=right:EnergyHungry] {}; \\\\" + lnSep);
			legend[7] = false;
		}
		if (legend[9]) {
			check = true;
			myString.append("		\\node [energyeffalt,label=right:EnergyEffAlt] {}; \\\\" + lnSep);
			legend[9] = false;
		}
		if (legend[10]) {
			check = true;
			myString.append("		\\node [contexmodeling,label=right:ContextModeling] {}; \\\\" + lnSep);
			legend[10] = false;
		}

		if (legend[8]) {
			check = true;
			myString.append("		\\node [energyfriendly,label=right:EnergyFriendly] {}; \\\\" + lnSep);
			legend[8] = false;
		}
		if (legend[2]) {
			check = true;
