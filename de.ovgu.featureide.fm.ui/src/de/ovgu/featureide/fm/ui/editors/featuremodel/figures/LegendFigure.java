...
import de.ovgu.featureide.fm.ui.editors.featuremodel.filters.ContextModelingGraphicalFeatureFilter;
import de.ovgu.featureide.fm.ui.editors.featuremodel.filters.EnergyEffAltGraphicalFeatureFilter;
import de.ovgu.featureide.fm.ui.editors.featuremodel.filters.EnergyFriendlyGraphicalFeatureFilter;
import de.ovgu.featureide.fm.ui.editors.featuremodel.filters.EnergyHungryGraphicalFeatureFilter;
...
import de.ovgu.featureide.fm.ui.editors.featuremodel.filters.PrimaryGraphicalFeatureFilter;
import de.ovgu.featureide.fm.ui.editors.featuremodel.filters.SecondaryGraphicalFeatureFilter;
import de.ovgu.featureide.fm.ui.editors.featuremodel.filters.UserGraphicalFeatureFilter;
...
public class LegendFigure extends Figure implements GUIDefaults {
	...
	private static final String ENERGYHUNGRY_TOOLTIP = "Energy Hungry feature:\n\nThis feature has impact at implementation level.";
	private static final String ENERGYFRIENDLY_TOOLTIP = "Energy Friendly feature:\n\nThis feature has impact at implementation level.";
	private static final String PRIMARY_TOOLTIP = "Primary Influencing Context";
	private static final String SECONDARY_TOOLTIP = "Secondary Influencing Context";
	private static final String USER_TOOLTIP = "User Influencing Context";
	private static final String CONTEXTMODELING_TOOLTIP = "ContextModeling feature:\n\nThis feature has impact at implementation level.";
	private static final String ENERGYEFFALT_TOOLTIP = "Energy Efficient Alternative feature:\n\nThis feature has impact at implementation level.";
	...
	private static final String DISALLOWED_EHUNGRY_TOOLTIP =
		"Disallwed energy hungry feature:\n\n This feature is declared mandatory and energy hungry,but does not have energy effecient alternatives.";
	...
	private static final int ENERGYHUNGRY = 17;
	private static final int ENERGYFRIENDLY = 18;
	private static final int ENERGYEFFALT = 19;
	private static final int CONTEXTMODELING = 20;
	private static final int PRIMARY = 21;
	private static final int SECONDARY = 22;
	...
	private boolean energyhungry;
	private boolean primary;
	private boolean secondary;
	private boolean user;
	private boolean contextmodeling;
	private boolean energyfriendly;
	private boolean energyeffalt;
	...
	private void refreshProperties(IFeatureModel featureModel) {
		...
		energyhungry = Functional.toList(Functional.filter(graphicalVisibleFeatures, new EnergyHungryGraphicalFeatureFilter())).size() > 0;
		contextmodeling = Functional.toList(Functional.filter(graphicalVisibleFeatures, new ContextModelingGraphicalFeatureFilter())).size() > 0;
		energyfriendly = Functional.toList(Functional.filter(graphicalVisibleFeatures, new EnergyFriendlyGraphicalFeatureFilter())).size() > 0;
		energyeffalt = Functional.toList(Functional.filter(graphicalVisibleFeatures, new EnergyEffAltGraphicalFeatureFilter())).size() > 0;
		hidden = Functional.toList(Functional.filter(graphicalVisibleFeatures, new HiddenGraphicalFeatureFilter())).size() > 0;
		primary = Functional.toList(Functional.filter(graphicalVisibleFeatures, new PrimaryGraphicalFeatureFilter())).size() > 0;
		secondary = Functional.toList(Functional.filter(graphicalVisibleFeatures, new SecondaryGraphicalFeatureFilter())).size() > 0;
		user = Functional.toList(Functional.filter(graphicalVisibleFeatures, new UserGraphicalFeatureFilter())).size() > 0;
		...
		disehungry = fmStructure.hasDisEhungry();
	...
	private void setLegendSize() {
		...
		if (energyeffalt) {
			height = height + ROW_HEIGHT;
			setWidth(language.getEnergyEffAlt());
		}
		if (energyhungry && !energyfriendly) {
			height = height + ROW_HEIGHT;
			setWidth(language.getEnergyHungry());
		}
		if (!energyhungry && energyfriendly) {
			height = height + ROW_HEIGHT;
			setWidth(language.getEnergyFriendly());
		}
		if (energyhungry && energyfriendly) {
			height = height + ROW_HEIGHT;
			setWidth(language.getEnergyFriendly());
		...
		if (disehungry) {
			height = height + ROW_HEIGHT;
			setWidth(language.getDisEhungry());
		}
		if (tautologyConst) {
			height = height + ROW_HEIGHT;
		...
		if (primary) {
			height = height + ROW_HEIGHT;
			setWidth(language.getPrimary());
		}
		if (secondary) {
			height = height + ROW_HEIGHT;
			setWidth(language.getSecondary());
		}
		if (user) {
			height = height + ROW_HEIGHT;
			setWidth(language.getUser());
		}
	...
	private void createRows() {
		...
		if (primary) {
			createRowPrimary(row++);
		}
		if (secondary) {
			createRowSecondary(row++);
		}
		if (user) {
			createRowUser(row++);
		}
		...
		if (energyfriendly && !energyhungry) {
			createRowEnergyFriendly(row++);
		}
		if (!energyfriendly && energyhungry) {
			createRowEnergyHungry(row++);
		}
	...
	private void createRowDisEhungry(int row) {
		createSymbol(row, FALSE_OPT, true, DISALLOWED_EHUNGRY_TOOLTIP);
		final Label labelDisEhungry = createLabel(row, language.getDisEhungry(), FMPropertyManager.getFeatureForgroundColor(), DISALLOWED_EHUNGRY_TOOLTIP);
		add(labelDisEhungry);
	}
	...
	private void createRowEffAlt(int row) {
		createSymbol(row, ENERGYEFFALT, true, ENERGYEFFALT_TOOLTIP);
		final Label labelEffAlt = createLabel(row, language.getEnergyEffAlt(), FMPropertyManager.getFeatureForgroundColor(), ENERGYEFFALT_TOOLTIP);
		add(labelEffAlt);
	}
	...
	private void createRowEnergyHungry(int row) {
		createSymbol(row, ENERGYHUNGRY, true, ENERGYHUNGRY_TOOLTIP);
		final Label labelEnergyHungry = createLabel(row, language.getEnergyHungry(), FMPropertyManager.getFeatureForgroundColor(), ENERGYHUNGRY_TOOLTIP);
		add(labelEnergyHungry);
	}

	private void createRowPrimary(int row) {
		createSymbol(row, PRIMARY, true, PRIMARY_TOOLTIP);
		final Label labelPrimary = createLabel(row, language.getPrimary(), FMPropertyManager.getFeatureForgroundColor(), PRIMARY_TOOLTIP);
		add(labelPrimary);
	}

	private void createRowSecondary(int row) {
		createSymbol(row, SECONDARY, true, SECONDARY_TOOLTIP);
		final Label labelSecondary = createLabel(row, language.getSecondary(), FMPropertyManager.getFeatureForgroundColor(), SECONDARY_TOOLTIP);
		add(labelSecondary);
	}

	private void createRowUser(int row) {
		createSymbol(row, USER, true, USER_TOOLTIP);
		final Label labelUser = createLabel(row, language.getUser(), FMPropertyManager.getFeatureForgroundColor(), USER_TOOLTIP);
		add(labelUser);
	}

	private void createRowContextModeling(int row) {
		createSymbol(row, CONTEXTMODELING, true, CONTEXTMODELING_TOOLTIP);
		final Label labelContextModeling =
			createLabel(row, language.getContextModeling(), FMPropertyManager.getFeatureForgroundColor(), CONTEXTMODELING_TOOLTIP);
		add(labelContextModeling);
	}

	private void createRowEnergyFriendly(int row) {
		createSymbol(row, ENERGYFRIENDLY, true, ENERGYFRIENDLY_TOOLTIP);
		final Label labelEnergyFriendly = createLabel(row, language.getEnergyFriendly(), FMPropertyManager.getFeatureForgroundColor(), ENERGYFRIENDLY_TOOLTIP);
		add(labelEnergyFriendly);
	}
	...
	private void createRowFeatureEnergyHungry(int row) {
		createSymbol(row, FEATURECON, true, FEATURE_TOOLTIP);
		final Label labelFeature = createLabel(row, language.getFeature(), FMPropertyManager.getFeatureForgroundColor(), FEATURE_TOOLTIP);
		add(labelFeature);
	}

	private void createRowFeatureEnergyFriendly(int row) {
		createSymbol(row, FEATURECON, true, FEATURE_TOOLTIP);
		final Label labelFeature = createLabel(row, language.getFeature(), FMPropertyManager.getFeatureForgroundColor(), FEATURE_TOOLTIP);
		add(labelFeature);
	}
	...
	private void createSymbol(int row, int type, boolean feature, String toolTip) {
		...
		case (CONCRETE):
			...
			break;
		case (ENERGYHUNGRY):
			rect.setBorder(FMPropertyManager.getEnergyHungryFeatureBorder(false));
			rect.setBackgroundColor(FMPropertyManager.getEnergyHungryFeatureBackgroundColor());
			break;
		case (PRIMARY):
			rect.setBorder(FMPropertyManager.getPrimaryFeatureBorder(false));
			rect.setBackgroundColor(FMPropertyManager.getPrimaryFeatureBackgroundColor());
			break;
		case (SECONDARY):
			rect.setBorder(FMPropertyManager.getSecondaryFeatureBorder(false));
			rect.setBackgroundColor(FMPropertyManager.getSecondaryFeatureBackgroundColor());
			break;
		case (USER):
			rect.setBorder(FMPropertyManager.getUserFeatureBorder(false));
			rect.setBackgroundColor(FMPropertyManager.getUserFeatureBackgroundColor());
			break;
		case (CONTEXTMODELING):
			rect.setBorder(FMPropertyManager.getContextModelingFeatureBorder(false));
			rect.setBackgroundColor(FMPropertyManager.getContextModelingFeatureBackgroundColor());
			break;
		case (ENERGYFRIENDLY):
			rect.setBorder(FMPropertyManager.getEnergyFriendlyFeatureBorder(false));
			rect.setBackgroundColor(FMPropertyManager.getEnergyFriendlyFeatureBackgroundColor());
			break;
		case (ENERGYEFFALT):
			rect.setBorder(FMPropertyManager.getEnergyEffAltFeatureBorder(false));
			rect.setBackgroundColor(FMPropertyManager.getEnergyEffAltFeatureBackgroundColor());
			break;
		case (FEATURECON):
			rect.setBorder(FMPropertyManager.getConcreteFeatureBorder(false));
			rect.setBackgroundColor(FMPropertyManager.getConcreteFeatureBackgroundColor());
