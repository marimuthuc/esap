...
public class ViewContentProvider implements IStructuredContentProvider, ITreeContentProvider, GUIDefaults {
	...
	protected static final String NUMBER_ENERGYHUNGRY = "Number of energy hungry features: ";
	protected static final String NUMBER_PRIMARY = "Number of primary influencing contexts ";
	protected static final String NUMBER_SECONDARY = "Number of secondary influencing contexts ";
	protected static final String NUMBER_USER = "Number of user influencing contexts ";
	protected static final String NUMBER_CONTEXTMODELING = "Number of contextmodeling features: ";
	protected static final String NUMBER_ENERGYFRIENDLY = "Number of energy friendly features: ";
	protected static final String NUMBER_ENERGYEFFALT = "Number of energy efficient Alternative features: ";
	...
	private static final int INDEX_ENERGYHUNGRY = 10;
	private static final int INDEX_PRIMARY = 14;
	private static final int INDEX_SECONDARY = 15;
	private static final int INDEX_USER = 16;
	private static final int INDEX_CONTEXTMODELING = 13;
	private static final int INDEX_ENERGYFRIENDLY = 11;
	private static final int INDEX_ENERGYEFFALT = 12;
	...
	private void addStatistics(TreeParent root, final String text, final IFeatureModel model, int position, boolean init, IProgressMonitor monitor) {
		...
		final int energyhungry = model.getAnalyser().countEnergyHungryFeatures();
		final int primary = model.getAnalyser().countPrimary();
		final int secondary = model.getAnalyser().countSecondary();
		final int user = model.getAnalyser().countUser();
		final int contextmodeling = model.getAnalyser().countContextModeling();
		final int energyeffalt = model.getAnalyser().countEnergyEffAltFeatures();
		final int energyfriendly = model.getAnalyser().countEnergyFriendlyFeatures();
		...
		if (init) {
			...
			final TreeParent statistics = new TreeParent(text, null, true) {
				...
				public void initChildren() {
					...
					addChild(NUMBER_ENERGYHUNGRY + energyhungry);
					addChild(NUMBER_PRIMARY + primary);
					addChild(NUMBER_SECONDARY + secondary);
					addChild(NUMBER_USER + user);
					addChild(NUMBER_ENERGYEFFALT + energyeffalt);
					addChild(NUMBER_ENERGYFRIENDLY + energyfriendly);
					addChild(NUMBER_CONTEXTMODELING + contextmodeling);
			...
			((TreeObject) children[INDEX_ENERGYHUNGRY]).setName(NUMBER_ENERGYHUNGRY + energyhungry);
			((TreeObject) children[INDEX_PRIMARY]).setName(NUMBER_PRIMARY + primary);
			((TreeObject) children[INDEX_SECONDARY]).setName(NUMBER_SECONDARY + secondary);
			((TreeObject) children[INDEX_USER]).setName(NUMBER_USER + user);
			((TreeObject) children[INDEX_CONTEXTMODELING]).setName(NUMBER_CONTEXTMODELING + contextmodeling);
			((TreeObject) children[INDEX_ENERGYEFFALT]).setName(NUMBER_ENERGYEFFALT + energyeffalt);
			((TreeObject) children[INDEX_ENERGYFRIENDLY]).setName(NUMBER_ENERGYFRIENDLY + energyfriendly);
