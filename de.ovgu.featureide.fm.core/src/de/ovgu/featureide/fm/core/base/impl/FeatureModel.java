...
public class FeatureModel implements IFeatureModel {
	...
	public void createDefaultValues(CharSequence projectName) {
		...
		addFeature(feature);
		// structure.getRoot().changeToOr();
		structure.getRoot().addChild(feature.getStructure());
		structure.getRoot().setAbstract(true);
		// structure.getRoot().changeToOr(); // Vinith

	}

	@Override
	public void createComposerValues(CharSequence projectName, String composer) {
		String rootName = getValidJavaIdentifier(projectName);
		if (rootName.isEmpty()) {
			rootName = "Root";
		}
		...
		final IFeature feature = new Feature(this, "Base");
		addFeature(feature);
		if (composer.equals("de.ovgu.featureide.core.ContextModeling")) {

			structure.getRoot().setComposerContext(true);
		}
		// structure.getRoot().changeToOr();
		structure.getRoot().addChild(feature.getStructure());
		structure.getRoot().setAbstract(true);
