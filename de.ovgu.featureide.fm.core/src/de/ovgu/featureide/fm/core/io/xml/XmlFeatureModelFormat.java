...
import static de.ovgu.featureide.fm.core.localization.StringTable.COMPOSERCONTEXT;
import static de.ovgu.featureide.fm.core.localization.StringTable.ENERGYHUNGRY;
...
import static de.ovgu.featureide.fm.core.localization.StringTable.PRIMARY;
import static de.ovgu.featureide.fm.core.localization.StringTable.SECONDARY;
import static de.ovgu.featureide.fm.core.localization.StringTable.USER;
...
public class XmlFeatureModelFormat extends AXMLFormat<IFeatureModel> implements IFeatureModelFormat {
	...
	public XmlFeatureModelFormat() {
		// System.out.println("InsidePlain");
	}

	public XmlFeatureModelFormat(IProject project) {
		proj = project;
		// System.out.println("InsideArg");
	}
	...
	private void parseFeatures(NodeList nodeList, IFeature parent) throws UnsupportedModelException {
		for (final Element e : getElements(nodeList)) {
			...
			boolean _ehungry = false;
			final boolean _contextmod = false;
			boolean composercont = false;
			...
			if (e.hasAttributes()) {
				...
				for (int i = 0; i < nodeMap.getLength(); i++) {
					...
					if (attributeName.equals(ABSTRACT)) {
						...
						// Legacy case, for backwards compatibility
					} else if (attributeName.equals(ENERGYHUNGRY)) {
						_ehungry = attributeValue.equals(TRUE);
					} else if (attributeName.equals(COMPOSERCONTEXT)) {
						composercont = attributeValue.equals(TRUE);
					} else if (attributeName.equals(PRIMARY)) {
						primary = attributeValue.equals(TRUE);
					} else if (attributeName.equals(SECONDARY)) {
						secondary = attributeValue.equals(TRUE);
					} else if (attributeName.equals(USER)) {
						user = attributeValue.equals(TRUE);
					} else {
			...
			f.getStructure().setEnergyHungry(_ehungry);
			f.getStructure().setComposerContext(composercont);
			if (primary) {
				f.getStructure().setPrimary();
			} else if (secondary) {
				f.getStructure().setSecondary();
			} else if (user) {
				f.getStructure().setUser();
			}
	...
	private void writeAttributes(Element node, Element fnod, IFeature feat) {
		...
		if (feat.getStructure().isEnergyHungry()) {
			fnod.setAttribute(ENERGYHUNGRY, TRUE);
		}
		if (feat.getStructure().isPrimary()) {
			fnod.setAttribute(PRIMARY, TRUE);
		} else if (feat.getStructure().isSecondary()) {
			fnod.setAttribute(SECONDARY, TRUE);
		} else if (feat.getStructure().isUser()) {
			fnod.setAttribute(USER, TRUE);
		}
		if (feat.getStructure().isComposerContext()) {
			fnod.setAttribute(COMPOSERCONTEXT, TRUE);
		}
